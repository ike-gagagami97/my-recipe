...
| ⑤の開始 | **エージェントが自分から**（L4 OK / マージ後。人間の明示依頼を待たない） |
...
| ⑤ 振り返り | エージェント中心 | L4 OK / マージ後に **自分から** skill/rule/workflow へ反映 |
...
**着手前ゲート（満たさなければ実装しない）:** feature doc の状態が「承認済み」以上、かつ「承認者」に人間の名前がある。未承認なら②へ戻る。
...
- [ ] **続けて⑤を行う**（人間に「振り返りして」と言われるのを待たない）
...
同じ問題を仕組みとして防ぐ。「エージェントの習慣」で片づけない。

### いつ始めるか（エージェントが自分で開始する）

人間から「⑤をして」と明示されなくても、次のいずれかで **⑤を開始する**:

1. 人間が **L4 OK** と言ったあと（マージ待ちでも振り返りの下書き・skill 反映は進めてよい）
2. 人間が **マージした** と伝えた／PR が merged になったのを確認したとき
3. feature の実装 PR が main に入り、vision / feature doc を「完了」に更新するタイミング

⑤をスキップして次 feature（①/②）に進まない。次機能の着手は⑤の出口を満たしてから。

### やること

1. 今回の作業で気づいた反省点・改善点を洗い出す（feature doc 付録の振り返り表に残してよい）
2. 「一度きり」か「繰り返しリスク」かを判断する
3. **繰り返しリスク** → skill / rule / workflow のいずれかに反映する（feature doc だけに書いて終わりにしない）
4. **一度きり** → Issue / PR コメントに留める（docs は変更しない）
5. 反映したパスを feature doc の振り返り表「反映先」に書く（または短い follow-up PR）

### 判断基準

| 判断 | 条件の例 |
| --- | --- |
| 繰り返しリスク | 「次の feature でも同じ手順を踏む」「見落としやすい確認がある」 |
| 一度きり | 「初回セットアップ特有」「そのプロジェクト固有の偶発事象」 |

### 出口（⑤完了）

- [ ] 反省点を洗い出した
- [ ] 繰り返しリスクがあれば **skill / rule / workflow** に反映した（feature doc のみは不可）
- [ ] 一度きりと判断したものはその理由を一言残した
- [ ] 人間に⑤の実施を依頼されなくても完了している
