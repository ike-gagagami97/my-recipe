...
- いまの開発フローが **HITL（Human in the Loop）** であること、HOTL との違い
...
協業の型で言うと、いまのフローは **HITL（Human in the Loop）** です（次節）。
...
| **HOTL** | Human **on** the Loop | 人間は**監視・介入役**としてループの外（上）にいる。通常は自動で進み、異常時に人が止まる |

イメージ:

```text
HITL:  エージェント → [人間の承認] → エージェント → [人間の受け入れ] → …
         ↑ 人がいないと進まないゲートがある

HOTL:  エージェント ⇄ エージェント ⇄ …（自動）
                    ↑
              人間がダッシュボード等で監視し、必要なら割り込む
```
...
| QAの仕事感 | 契約承認・Preview受け入れが本業 | アラート対応・サンプリング監査・仕組みの穴探しが本業になりやすい |

どちらが「上級」という話ではありません。**リスクと自律度のトレードオフ**です。

### 2.3 このリポジトリは HITL

いまの①〜⑤は、次の**人間ゲート**があるため HITL です。
...
| ③ 短い依頼 | 人間 | 実装開始のトリガーは人 |
| L4 Preview | 人間（QA） | 新規画面・新規フロー等は人が受け入れ |
| Production マージ（D1） | **人間のみ** | エージェントは draft PR まで。本番投入は人が決める |

一方で、ゲートの**あいだ**はエージェントに寄せています（HITL でも自動化は使う）:

- ④の実装・自己検証・L0〜L3・draft PR
- サブエージェントによる独立判定（doc review / code review / DB監査 / acceptance）
- ⑤振り返り（L4 OK / マージ後にエージェントが自分から開始）

```mermaid
flowchart TD
  H1[人間ゲート: ①決める / ②承認 / ③依頼]
  A1[エージェント: ④実装・L0〜L3・draft PR]
...
- feature doc 未承認でも実装が進む、は許容していない
- Preview 受け入れや Production マージをエージェント任せにしない
- 「異常が出たら人が止める」監視モデルではなく、「人が OK するまで進まない」ゲートモデル

### 2.4 将来 HOTL に寄せるなら（参考）

いまは意図的に HITL です。もし将来、低リスク変更だけ HOTL 寄りにするなら、例えば次が揃ってからになります。

- 振る舞い不変の docs 変更などは既に L0 のみ（部分的な自動化）
- さらに進めるなら: 自動 E2E のカバレッジ、失敗時の自動ロールバック、監視・アラート、サンプリング監査の設計
- 認証・権限・削除・新規フローなど高リスクは、しばらく HITL（L4 + 人間マージ）のままが安全
...
このフロー全体が **HITL** です（§2）。ゲートのあいだはエージェントが進め、ゲートでは人が止めます。
...
| これは HOTL（人が監視するだけ） | いまはゲート型の HITL。人が OK しないと進まない |
